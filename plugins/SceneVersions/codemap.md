...
```
On read, the string is `JSON.parse`'d back to `string[]`. Links are **bidirectional**: when scene A lists scene B, scene B also lists scene A. The `syncBidirectional` function updates both sides using **best-effort** semantics: peer writes run concurrently via `Promise.allSettled`, individual peer failures are logged to `console.error` and do not block the remaining peers, and the current scene's own list is written last and unconditionally (so a peer write failure can never leave the user's edit unsaved).
...
- **Bidirectional sync**: `syncBidirectional` reads each added/removed scene's current `RelatedScenes`, patches it, and writes back. Peer writes run concurrently with best-effort error handling (one failure does not block the others); the current scene's list is written last and unconditionally. Self-links are filtered at every step.
...
4. **Picker selection** (`handleSelect`): replaces `relatedIds` with the selected scene IDs and `relatedScenes` with the selected scene objects (so the card list updates immediately, matching `handleRemove`/`handleAddSuggestion`), sets `dirty` by comparing to `loadedIdsRef.current`.
...
## Files
- `SceneVersions.yml` — plugin manifest (name, description, version 0.4.2, `ui.javascript`/`ui.css`).
