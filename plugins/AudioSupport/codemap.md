...
## Files
- `AudioSupport.yml` — plugin manifest (name, description, version 0.5.0, `interface: js`, `exec: AudioSupportHook.js`, `hooks: Scene.Create.Post`, `tasks: tagAll`, `ui.requires: CommunityScriptsUILibrary`, JS/CSS assets).
- `AudioSupport.js` — vanilla-JS player overlay. Registers `PluginApi.patch.instead("ScenePlayer", ...)` at script load to replace video.js with a mount div (`#AudioSupportMount`) for audio-only scenes. Detects audio-only scenes via GraphQL (`video_codec === ""` or `width === 0`), renders an HTML5 `<audio>` element loading the direct stream (`scene.paths.stream` or `/scene/{id}/stream`). The overlay fills the entire `#AudioSupportMount` player area with a centered cover art + metadata + transport layout. Supports playback speed (0.5x–2x), loop toggle, keyboard shortcuts (Space/Arrows/m/l/k), volume persistence, and a synced LRC lyrics panel with load/edit, click-to-seek, and auto-scroll.
...
## Data model
- Plugin config key: `"AudioSupport"` (csLib). State shape: `{ collapsed, opacity, panelPos, audioTagName, playbackRate, loop, volume, showNavEntry, lyricsVisible }`.
- Scene lyrics are stored in `custom_fields.AudioLyrics` (configurable via `ui.settings.AudioLyricsField`, default `AudioLyrics`). The overlay loads, parses, and saves synced LRC text per scene using Apollo; lyrics are not persisted in the plugin config.
...
8. Settings page tabs:
   - **Browse** (default): sub-nav with three views:
     - **By Work**: groups audio scenes by parent directory; each work card shows the first scene's cover, work name (directory basename), chapter count, and total duration. Clicking a work opens a chapter list sorted by natural filename order, with a "Play Work" placeholder button and a back button.
     - **All Audio**: existing flat card grid with search/sort kept intact.
     - **By Tag**: collects all tags on audio scenes, shows tag cards with scene counts; clicking a tag shows its scene grid with a "Play All" placeholder button and a back button.
